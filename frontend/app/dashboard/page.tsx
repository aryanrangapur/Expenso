...
export default function DashboardPage() {
  ...
  const loadExpenses = async () => {
    try {
      // For now, using dummy data since we're focusing on auth
      // You can integrate your actual API calls here later
      setTimeout(() => {
        setExpenses([]) // Empty array for demo
        setLoading(false)
      }, 1000)
    } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Welcome back{user?.firstName ? `, ${user.firstName}` : ''}! Here's your expense overview.
          </p>
        </div>
